...
func LoadProfile(id *ID, profileStore *ProfileStore) (*Profile, error) {
	...
	if v, err := profileStore.backend.CommittedStore().Get(pathBuiler.ProfileField("")); v == nil || err != nil {
		return nil, err
	}
	...
	if paraDegree, err := profileStore.backend.CommittedStore().Get(path); paraDegree != nil && err == nil {
	...
	if prepayment, err := profileStore.backend.CommittedStore().Get(path); prepayment != nil && err == nil {
	...
	if Indices, err := profileStore.backend.CommittedStore().Get(path); Indices != nil && err == nil {
