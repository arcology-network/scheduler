...
func (this *Scheduler) RegisterConflict(lftAddr [20]byte, lftSig [4]byte, rgtAddr [20]byte, rgtSig [4]byte) bool {
	seedCallee := this.GetOrCreateProfile(lftAddr, lftSig)
	otherCallee := this.GetOrCreateProfile(rgtAddr, rgtSig)

	if seedCallee.IsInConflictList(otherCallee.UID) && otherCallee.IsInConflictList(seedCallee.UID) {
...
func (this *Scheduler) AddDeferred(addr [20]byte, selector [4]byte) {
	callee := this.GetOrCreateProfile(addr, selector)
